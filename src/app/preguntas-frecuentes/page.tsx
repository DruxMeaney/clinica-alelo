    ...
    <>
      <section className="bg-gradient-to-br from-white via-gray-50 to-emerald-50/30 py-20">
        <div className="max-w-4xl mx-auto px-6">
          <p className="text-sm font-medium text-[#2D6A4F] tracking-widest uppercase mb-4">FAQ</p>
              ...
              <div className="space-y-4">
                {FAQS.filter((f) => f.category === cat).map((faq) => (
                  <details key={faq.question} className="group rounded-2xl border border-gray-100 bg-white">
                    <summary className="px-6 py-4 cursor-pointer text-gray-900 font-medium text-sm hover:text-[#2D6A4F] transition-colors list-none flex justify-between items-center">
                      {faq.question}
